...
/**
 * Clip a polygon (outer ring + holes) to a rectangular extent.
 * Rings are clipped independently with Sutherland–Hodgman. Prefer
 * {@link clipFlatTriangleToExtent} after earcut for concave polygons with
 * holes — clipping rings first can self-intersect and confuse triangulation.
 *
 * @param {Array<number>|Float32Array} flatCoordinates Interleaved XY rings.
 * @param {Array<number>} holes Earcut hole indices (vertex index at each hole start).
 * @param {import("../../extent.js").Extent} extent Clip rectangle.
 * @return {{flatCoordinates: Array<number>, holes: Array<number>}|null} Clipped
 *     polygon, or null if the outer ring is empty after clipping.
 */
export function clipFlatPolygonToExtent(flatCoordinates, holes, extent) {
  /** @type {Array<number>} */
  const ends = holes.slice();
  ends.push(flatCoordinates.length / 2);

  /** @type {Array<number>} */
  const outer = [];
  for (let i = 0; i < ends[0]; ++i) {
    outer.push(flatCoordinates[i * 2], flatCoordinates[i * 2 + 1]);
  }
  const clippedOuter = clipFlatRingToExtent(outer, extent);
  if (clippedOuter.length < 6) {
    return null;
  }

  /** @type {Array<number>} */
  const dest = clippedOuter.slice();
  /** @type {Array<number>} */
  const destHoles = [];
  let vertexCount = clippedOuter.length / 2;
  ...
  for (let h = 0; h < holes.length; ++h) {
    ...
    for (let i = start; i < end; ++i) {
      hole.push(flatCoordinates[i * 2], flatCoordinates[i * 2 + 1]);
    }
    // Only keep holes fully inside the clip. Boundary-crossing holes become
    // topology errors if clipped independently (hole edges on the outer rim).
    ...
    destHoles.push(vertexCount);
    dest.push(...hole);
    vertexCount += hole.length / 2;
  }

  return {flatCoordinates: dest, holes: destHoles};
}
